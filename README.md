...
For each `group_id`, we sort samples by $n_{\mathrm{mut}}$ and apply a hinge loss over adjacent pairs only (for efficiency). Let $d_i = d(u_{\mathrm{pcc}}(r_i), u_{\mathrm{pcc}}(v_i))$. For adjacent items with $\Delta n = n_{i+1} - n_i > 0$ and $\Delta d = d_{i+1} - d_i$:
